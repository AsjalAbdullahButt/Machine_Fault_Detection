...
def build_binary_lstm(input_shape, units=LSTM_UNITS, dropout=DROPOUT_RATE, learning_rate=LEARNING_RATE):
    """Two-layer stacked LSTM for binary fault classification (sigmoid output)."""
    model = Sequential(
        [
...
def build_multiclass_lstm(input_shape, num_classes, units=LSTM_UNITS, dropout=DROPOUT_RATE, learning_rate=LEARNING_RATE):
    """Two-layer stacked LSTM for multiclass failure-type classification (softmax output)."""
    model = Sequential(
        [
            LSTM(units, return_sequences=True, input_shape=input_shape),
            BatchNormalization(),
...
def build_binary_cnn_lstm(input_shape, units=LSTM_UNITS, dropout=DROPOUT_RATE, learning_rate=LEARNING_RATE):
    """Conv1D feature extraction followed by LSTM for binary classification."""
    model = Sequential(
        [
...
def build_multiclass_cnn_lstm(input_shape, num_classes, units=LSTM_UNITS, dropout=DROPOUT_RATE, learning_rate=LEARNING_RATE):
    """Conv1D feature extraction followed by LSTM for multiclass classification."""
    model = Sequential(
        [
            Conv1D(64, kernel_size=3, activation="relu", padding="same", input_shape=input_shape),
            MaxPooling1D(pool_size=2),
...
def _build_transformer_backbone(input_shape, d_model: int = 64, heads: int = 2, dropout: float = DROPOUT_RATE):
    ...
    attn = MultiHeadAttention(
        num_heads=heads, key_dim=d_model // heads, dropout=dropout, name="self_attention",
    )(x, x)
    x = Add()([x, attn])
...
def train_model(
    ...
    callbacks = [
        EarlyStopping(
            ...
            min_delta=ES_MIN_DELTA, restore_best_weights=True, verbose=1,  # type: ignore[arg-type]
        ),
        ReduceLROnPlateau(
    ...
    if history_save_path:
        os.makedirs(
            os.path.dirname(history_save_path) if os.path.dirname(history_save_path) else ".",
            exist_ok=True,
        )
...
def load_history_from_json(json_path: str):
    """Load a saved training history dict from JSON; returns None if file is missing."""
    ...
    h = types.SimpleNamespace(
        ...
        epoch   = data.get("epoch", list(range(len(next(iter(history_data.values()), []))))),
...
def _save_fig(path: str, fig):
    """Save matplotlib figure to path, creating parent dirs as needed."""
...
def _resolve_plot_path(plot_id: str) -> str:
    """Accept either a PLOT_NAMES key (short string) or a direct absolute path."""
    try:
...
def plot_training_history(history, title: str, plot_key: str):
    ...
    if not history_dict or "loss" not in history_dict:
        # Try loading from the JSON saved alongside the model
        json_path = path.replace(".png", "_history.json").replace(
            "plots" + os.sep, "models" + os.sep
        )
    ...
    if not history_dict or "loss" not in history_dict:
        ...
        ax.text(0.5, 0.5,
                f"Training history not available for:\n{title}\n\nRun training first (not --load-models)",
    ...
    if "loss" in history_dict and "val_loss" in history_dict:
        if len(epochs) == 1:
            axes[0].scatter(epochs, history_dict["loss"],     label="Train Loss", s=60)
            axes[0].scatter(epochs, history_dict["val_loss"], label="Val Loss",   s=60, marker="x")
            axes[0].set_xlim(0.5, 1.5); axes[0].set_xticks([1])
        else:
        ...
        axes[0].text(0.5, 0.5, "Loss history unavailable", ha="center", va="center",
                     transform=axes[0].transAxes)
        axes[0].set_axis_off()

    # Accuracy subplot (handles both 'accuracy' and 'binary_accuracy' key names)
    acc_key = next(
        (k for k in ["accuracy", "binary_accuracy"] if k in history_dict), None
    )
    val_key = f"val_{acc_key}" if acc_key else None
    if acc_key and val_key and val_key in history_dict:
        if len(epochs) == 1:
            axes[1].scatter(epochs, history_dict[acc_key], label="Train Acc", s=60)
            axes[1].scatter(epochs, history_dict[val_key], label="Val Acc",   s=60, marker="x")
            axes[1].set_xlim(0.5, 1.5); axes[1].set_xticks([1])
        else:
            axes[1].plot(epochs, history_dict[acc_key], label="Train Acc", linewidth=2)
            axes[1].plot(epochs, history_dict[val_key], label="Val Acc",   linewidth=2, linestyle="--")
        axes[1].set_title("Accuracy over Epochs")
        axes[1].set_xlabel("Epoch"); axes[1].set_ylabel("Accuracy")
        axes[1].legend(); axes[1].grid(alpha=0.3)
    else:
        axes[1].text(0.5, 0.5, "Accuracy history unavailable", ha="center", va="center",
...
def plot_roc_binary(y_true, y_prob, plot_key: str):
    ...
    ax.set_title("Binary — ROC Curve", fontweight="bold")
...
def plot_roc_multiclass(y_true, y_prob, class_names, plot_key: str):
    ...
    ax.set_xlim(0.0, 1.0); ax.set_ylim(0.0, 1.02)
    ax.set_xlabel("False Positive Rate"); ax.set_ylabel("True Positive Rate")
    ax.set_title("Multiclass — One-vs-Rest ROC Curves", fontweight="bold")
...
def plot_pr_binary(y_true, y_prob, plot_key: str, title: str = "Binary — Precision-Recall Curve"):
    ...
    fig, ax = plt.subplots(figsize=(7, 5))
    ax.plot(recall, precision, color="#2c7fb8", lw=2, label=f"PR curve (AP = {ap_score:.3f})")
    baseline = np.mean(y_true)
    ax.hlines(baseline, 0, 1, colors="#7f8c8d", linestyles="--", label=f"Baseline={baseline:.3f}")
    ax.set_xlim(0.0, 1.0); ax.set_ylim(0.0, 1.02)
...
def plot_pr_multiclass(y_true, y_prob, class_names, plot_key: str, title: str = "Multiclass — One-vs-Rest PR Curves"):
    ...
    y_bin_arr = np.asarray(y_bin)
    fig, ax = plt.subplots(figsize=(9, 6))
    for i, name in enumerate(class_names):
        if y_bin_arr[:, i].sum() == 0:
            continue
        precision, recall, _ = precision_recall_curve(y_bin_arr[:, i], y_prob[:, i])
        ap = average_precision_score(y_bin_arr[:, i], y_prob[:, i])
    ...
    _save_fig(path, fig)
    return average_precision_score(y_bin_arr, y_prob, average="weighted")
...
def plot_calibration_binary(y_true, y_prob_raw, y_prob_used, plot_key: str, title: str = "Binary — Calibration Curve"):
    ...
    ax.plot(mean_pred_raw,  frac_pos_raw,  marker="o", label="Raw probabilities")
    if not np.allclose(y_prob_raw, y_prob_used):
        ax.plot(mean_pred_used, frac_pos_used, marker="s", label="Platt-scaled probabilities")
...
def evaluate_binary(
    ...
    if plot_ids is None:
        plot_ids = {
            "roc": "binary_roc", "pr": "binary_pr",
            "calibration": "binary_calibration",
            "cm": "binary_cm",  "f1": "binary_f1",
        }
    ...
    pr_auc  = plot_pr_binary(y_test, y_prob_used, plot_ids["pr"],
                              title=f"{model_name} — Precision-Recall Curve")
    brier   = plot_calibration_binary(
        y_test, y_prob_raw, y_prob_used,
        plot_ids["calibration"], title=f"{model_name} — Calibration Curve",
    )
    ...
    plot_f1_bars(report_dict, f"{model_name} — F1 Score per Class", plot_ids["f1"])  # type: ignore[arg-type]
    print(sep)
...
def evaluate_multiclass(
    ...
    if plot_ids is None:
        plot_ids = {"roc": "multi_roc", "pr": "multi_pr", "cm": "multi_cm", "f1": "multi_f1"}
    ...
    present_all   = sorted(np.unique(np.concatenate([y_test, y_pred])))
    present_names = [class_names[i] for i in present_all if i < len(class_names)]

    print(f"\n  Classification Report:")
    print(classification_report(y_test, y_pred, labels=present_all, target_names=present_names, zero_division=0))

    report_dict = classification_report(
        y_test, y_pred, labels=present_all,
    ...
    plot_pr_multiclass(y_test, y_prob, class_names, plot_ids["pr"],
                       title=f"{model_name} — One-vs-Rest PR Curves")
    plot_f1_bars(report_dict, f"{model_name} — F1 Score per Class", plot_ids["f1"])  # type: ignore[arg-type]
    print(sep)
